...
def new_window():
    ...
    def process_features():
        ...
        for i in range(len(sel)):
            x=pd.DataFrame()
            if sel[i]==1:
                x=pd.read_csv('output/fset_prot_'+ types[i] +'.csv')
                ...
                try:
                    ...
                    Label(frame3_n, text="Empty Dropped for: "+types[i]).pack()
                except:
                    Label(frame3_n, text="Nothing Dropped for: "+types[i]).pack()
            else:
                pass
            print(x.shape,x_processed.shape)
            x_processed = pd.concat([x, x_processed], axis=1)
            
        
        Label(frame3_n, text="Features processed! Shape:"+str(x_processed.shape)).pack()
